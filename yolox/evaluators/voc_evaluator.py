...
class VOCEvaluator:
    ...
    def evaluate(
        ...
        tensor_type = torch.BFloat16Tensor if half else torch.FloatTensor # tensor_type = torch.cuda.HalfTensor if half else torch.cuda.FloatTensor
        model = model.eval()
        if half:
            model = model.half()
        ids = []
        ...
        if trt_file is not None:
            ...
            x = torch.ones(1, 3, test_size[0], test_size[1]) # x = torch.ones(1, 3, test_size[0], test_size[1]).cuda()
            model(x)
            model = model_trt
        ...
        statistics = torch.FloatTensor([inference_time, nms_time, n_samples]) # statistics = torch.cuda.FloatTensor([inference_time, nms_time, n_samples])
        if distributed:
